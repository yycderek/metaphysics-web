...
- 📊 **双模式展示**：起课后可切换「课式结果」（天地盘/四课/三传总览）与「🧭 推导过程」（逐步动画，可自动演示）
- 🧬 **智能占卜 Agent**（自主式）：直接描述问事（可带日期/时辰），Agent 自主选择算法与参数、调用引擎精确起课，再输出**结构化断语**（卦象/简略与详细/依据）；支持澄清追问、复读已算之卦、**多卦综断与换时辰对比表**、**断语二次审视自纠**、SSE 流式过程；预留**可选出生信息**用于个人化断课
...
- **引擎补算**：梅花自动判**体用关系**（动爻所在卦为用）、六爻/大六壬给出**用神与应期**提示（不再纯靠 LLM 猜）
...
npm test             # vitest 单元/组件测试（124 项，含易学/六爻/梅花/天文/Agent/应验/历史/评估/限流/断例/技能/安全/表单）
