...
// 贼克法
function zeke(
  rigan: string, rizhi: string, tianpan: Record<string, string>, sike: Sike, positions: number[],
): SanchuanResult {
...
function shehaiRank(
  tianpan: Record<string, string>, sike: Sike, positions: number[],
): number {
  ...
  for (const i of positions) {
    ...
    const score: [number, number, number] = [
      depth, MZJ_ORDER[DIZHI_MENGZHONGJI[bottom]], -i,
    ]
...
// 遥克法
function yaoke(
  rigan: string, rizhi: string, tianpan: Record<string, string>, sike: Sike,
): SanchuanResult | null {
