...
const VALID_RESULT = {
  ...
  steps: [
    { key: 'yue', title: '一、大安起月', desc: '顺数 3 个月。', data: { landed: '速喜' } },
  ],
...
describe('远程算法服务适配器', () => {
  it('成功路径：POST input → 解析 DivinationResult', async () => {
    ...
    const adapter = createRemoteAdapter({ id: 'xiaoliuren-remote', name: '小六壬(远程)', url: 'https://example.com/divine' })
  ...
  it('结构不合法（缺 algorithmId）→ 明确错误', async () => {
    mockFetchOnce(async () =>
      new Response(JSON.stringify({ steps: [] }), { status: 200, headers: { 'Content-Type': 'application/json' } }),
    )
  ...
  it('注册到注册表后 buildDivination 可异步调用', async () => {
    mockFetchOnce(async () =>
      new Response(JSON.stringify(VALID_RESULT), { status: 200, headers: { 'Content-Type': 'application/json' } }),
    )
    registerAdapter(createRemoteAdapter({ id: 'xiaoliuren-remote', name: '小六壬(远程)', url: 'https://example.com/divine' }))
