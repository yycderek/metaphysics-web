...
/** 读取 API key：环境变量 → ~/.hermes/.env → ~/.hermes/config.yaml */
...
export function resolveAIConfig(user?: UserAIConfig): AIProviderConfig {
  const envBaseUrl = process.env.DEEPSEEK_BASE_URL ?? DEFAULT_BASE_URL;
  const envModel = process.env.DEEPSEEK_MODEL ?? DEFAULT_MODEL;
  return {
