...
【通俗语言（务必遵守）】面向普通用户，用大白话讲，少用晦涩术语；专业词第一次出现时用括号用生活化比喻解释（例如：官鬼=工作压力与权威，父母=文书、长辈庇护，妻财=钱与利益，兄弟=同辈竞争，子孙=子女、福气与化解，旬空=暂时落空、时机未到；六亲=这六种关系，用神=所问之事对应的那一样）。结论要与上面的卦象、课式一致，讲清"为什么"，不要只堆口诀。${forcedBlock}${skillBlock}${calibrationBlock}${priorBlock}
