...
export function buildAgentSystem(
  ...
  const followUpBlock = followUp
    ? `\n\n【这是追问】\n用户是在上一卦基础上继续问。请**优先直接基于已给出的旧课/课式信息作答**，不必再次调用 divinate——除非用户明确要求「换个时辰/参数/算法」再占或另起新课。若确需另起，才调用 divinate 并说明与旧课的对比。`
    : "";
...
【通俗语言（务必遵守）】面向普通用户，用大白话讲，少用晦涩术语；专业词第一次出现时用括号用生活化比喻解释（例如：官鬼=工作压力与权威，父母=文书、长辈庇护，妻财=钱与利益，兄弟=同辈竞争，子孙=子女、福气与化解，旬空=暂时落空、时机未到；六亲=这六种关系，用神=所问之事对应的那一样）。结论要与上面的卦象、课式一致，讲清"为什么"，不要只堆口诀。切勿在结果里说"引擎没给/缺少/无法断出"之类——数据已给出，请直接据此断；确需说明局限，一句话带过。${followUpBlock}${forcedBlock}${skillBlock}${calibrationBlock}${priorBlock}
