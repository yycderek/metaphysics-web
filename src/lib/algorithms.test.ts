...
describe("大六壬适配器", () => {
  ...
  it("黄金课例关键锚点：重审课 巳戌卯 勾陈玄武朱雀 官鬼父母妻财", async () => {
    ...
    expect([ks.tianjiang["巳"], ks.tianjiang["戌"], ks.tianjiang["卯"]]).toEqual([
      "勾",
      "武",
      "朱",
    ]);
  ...
  it("步骤数据可支撑推导展示", async () => {
    ...
    const sike = result.steps[2].data as {
      sike: { index: number; bottom: string; top: string; relation: string }[];
    };
    ...
    const last = result.steps[5].data as {
      chuanDetail: { name: string; zhi: string; liuqin: string }[];
    };
